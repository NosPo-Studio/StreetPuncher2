...
function Exhaust.new(args)
	...
	this.particle = args.particle or "Blood"
	...
	this.upForce = pa(args.force, 0)
	this.upForceRange = pa(args.upForceRange, 0)
	this.sideForceRange = pa(args.sideForceRange, 0)
	...
	this.update = function(this, dt, ra) --will called on every game tick.
		...
		for c = 1, toSpawn do
			...
			local sideForce = math.random(-this.sideForceRange, this.sideForceRange)
			local upForce = math.random(-this.upForceRange, this.upForceRange)
			...
			particle.gameObject:setSpeed(sideForce, - this.upForce + upForce)
		end
	end
